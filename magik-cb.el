...
(defun magik-cb-filter (p s)
  ...
  (save-match-data
    (let* ((b (process-buffer p))
      ...
      (when (string-match "\C-e" magik-cb-filter-str) (magik-cb-read-methods p))
      (when (string-match "\C-u" magik-cb-filter-str) (magik-cb-force-query  p))
      (when (string-match "\C-c" magik-cb-filter-str) (magik-cb-read-classes p))
...
(defun magik-cb-sentinel (proc msg)
  (let ((status (process-status proc))
    ...
    (when (and (or (eq status 'exit) (eq status 'signal))
               (buffer-live-p buf))
      (with-current-buffer buf
...
(defun magik-cb-family (class)
  ...
  (when (and (stringp class) (not (equal class "")))
